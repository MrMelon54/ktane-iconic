...
public class FakeBombInfo : MonoBehaviour
{
    ...
    public List<NeedyTimer> needyModuleTimers
    {
        get { return needyModules.Select(x => x.GetComponentInChildren<NeedyTimer>()).ToList(); }
    }
    ...
    public void SetupEdgework(EdgeworkConfiguration config)
    {
        ...
        else
        {
            foreach (THWidget widgetConfig in config.Widgets)
            {
                ...
                else if (widgetConfig.Type == WidgetType.INDICATOR &&
                         widgetConfig.IndicatorLabel == IndicatorLabel.RANDOM)
                {
                    ...
                    switch (widgetConfig.Type)
                    {
                        case WidgetType.BATTERY:
                            for (int i = 0; i < widgetConfig.Count; i++)
                            {
                                ...
                                {
                                    widgets.Add(BatteryWidget.CreateComponent(BatteryWidget,
                                        widgetConfig.BatteryCount));
                                }
                                else if (widgetConfig.BatteryType == BatteryType.RANDOM)
                                {
                                    widgets.Add(BatteryWidget.CreateComponent(BatteryWidget,
                                        Random.Range(widgetConfig.MinBatteries, widgetConfig.MaxBatteries + 1)));
                                }
                                else
                                {
                                    widgets.Add(BatteryWidget.CreateComponent(BatteryWidget,
                                        (int)widgetConfig.BatteryType));
                                }
                            }
                        ...
                        case WidgetType.INDICATOR:
                            ...
                            {
                                widgets.Add(IndicatorWidget.CreateComponent(IndicatorWidget, widgetConfig.CustomLabel,
                                    widgetConfig.IndicatorState));
                            }
                            else
                            {
                                widgets.Add(IndicatorWidget.CreateComponent(IndicatorWidget,
                                    widgetConfig.IndicatorLabel.ToString(), widgetConfig.IndicatorState));
                            }
                        ...
                        case WidgetType.TWOFACTOR:
                            for (int i = 0; i < widgetConfig.Count; i++)
                            {
                                widgets.Add(TwoFactorWidget.CreateComponent(TwoFactorWidget,
                                    config.TwoFactorResetTime));
                            }
                        ...
                        case WidgetType.CUSTOM:
                            for (int i = 0; i < widgetConfig.Count; i++)
                            {
                                widgets.Add(CustomWidget.CreateComponent(CustomWidget, widgetConfig.CustomQueryKey,
                                    widgetConfig.CustomData));
                            }
...
public class TestHarness : MonoBehaviour
{
    ...
    public Dictionary<KMSoundOverride.SoundEffect, AudioSource> GameSoundEffectSources =
        new Dictionary<KMSoundOverride.SoundEffect, AudioSource>();

    public Dictionary<KMSoundOverride.SoundEffect, List<AudioClip>> GameSoundEffects =
    ...
    void LogErrorAtTransform(Transform obj, string unassigned)
    {
        ...
        Debug.LogErrorFormat("There is an unassigned {0} on the following object: {1}", unassigned,
    ...
    WidgetZone CreateWidgetArea(Vector3 rot, Vector3 pos, Vector3 scale, Transform parent, List<GameObject> areaList,
        string name)
    {
    ...
    Transform PrepareModuleAnchor(List<List<Transform>> anchors, Transform module, ref int timerFace, Transform parent,
        bool prepareTwitchPlays = true)
    {
    ...
    void PrepareBomb(List<KMBombModule> bombModules, List<KMNeedyModule> needyModules, ref List<Widget> widgets)
    {
        ...
        else
        {
            ...
            widgetZones.Add(CreateWidgetArea(new Vector3(0, 0, -90), new Vector3(0, 0, (-squaresize / 2) - 0.0175f),
                new Vector3(squaresize, 0.03f, 0.17f), widgetAreas, bomb.WidgetAreas, "Bottom Widget Area"));
            widgetZones.Add(CreateWidgetArea(new Vector3(-90, 90, 0), new Vector3((-squaresize / 2) - 0.0175f, 0, 0),
                new Vector3(squaresize, 0.03f, 0.17f), widgetAreas, bomb.WidgetAreas, "Lef Widget Area"));
            widgetZones.Add(CreateWidgetArea(new Vector3(-90, -90, 0), new Vector3((squaresize / 2) + 0.0175f, 0, 0),
                new Vector3(squaresize, 0.03f, 0.17f), widgetAreas, bomb.WidgetAreas, "Right Widget Area"));
            widgetZones.Add(CreateWidgetArea(new Vector3(0, -180, -90), new Vector3(0, 0, (squaresize / 2) + 0.0175f),
                new Vector3(squaresize, 0.03f, 0.17f), widgetAreas, bomb.WidgetAreas, "Top Widget Area"));
    ...
    void Handlers(KMBombInfo component)
    {
        ...
        component.SolvableModuleNamesHandler +=
            new KMBombInfo.GetSolvableModuleNamesHandler(fakeInfo.GetSolvableModuleNames);
        ...
        component.WidgetQueryResponsesHandler +=
            new KMBombInfo.GetWidgetQueryResponsesHandler(fakeInfo.GetWidgetQueryResponses);
    ...
    private KMAudio.KMAudioRef PlaySoundEffectHandler(KMSoundOverride.SoundEffect effect, Transform t)
    {
        ...
        AudioSource source;
        if (!GameSoundEffects.TryGetValue(effect, out clips) ||
            !GameSoundEffectSources.TryGetValue(effect, out source) || clips.Count == 0)
        {
            audioRef.StopSound = () => { };
            return audioRef;
        }
            ...
            if (rayCastHitSomething && !_interacting)
            {
                ...
                if (hitArea != null)
                {
                    if (currentSelectableArea != hitArea)
                    {
                        ...
                        {
                            PlaySoundEffectHandler(KMSoundOverride.SoundEffect.SelectionTick,
                                hitArea.Selectable.transform);
    ...
    public static IEnumerator MoveCamera(Transform destination)
    {
        ...
        Vector3 cameraDestination =
            new Vector3(destination.position.x, Instance._camera.localPosition.y, destination.position.z);
        Instance._bomb.localEulerAngles = bombOrigin;

        while ((Time.time - startTime) < moveTime)
        {
            Instance._bomb.rotation = Quaternion.Lerp(Quaternion.Euler(bombOrigin), Quaternion.Euler(bombDestination),
                (Time.time - startTime) / moveTime);
            Instance._camera.localPosition =
                Vector3.Lerp(cameraOrigin, cameraDestination, (Time.time - startTime) / moveTime);
            yield return null;
        }
